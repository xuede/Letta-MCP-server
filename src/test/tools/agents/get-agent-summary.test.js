...
} from '../../../tools/agents/get-agent-summary.js';
...
describe('Get Agent Summary', () => {
    ...
    describe('Functionality Tests', () => {
        it('should get agent summary successfully with all data', async () => {
            ...
            const mockCoreMemory = [
                ...
                {
                    label: 'persona',
                    value: 'I am a helpful AI assistant with expertise in programming.',
                },
            ];
            ...
            expect(parsedResult).toMatchObject({
                ...
                core_memory_blocks: [
                    {
                        label: 'human',
                        value_snippet: expect.stringContaining('software developer'),
                    },
                    {
                        label: 'persona',
                        value_snippet: expect.stringContaining('helpful AI assistant'),
                    },
                ],
                attached_tools_count: 2,
                attached_tools: mockTools.map((t) => ({
                    id: t.id,
                    name: t.name,
                    type: t.tool_type,
                })),
        ...
        it('should handle long system prompts and memory values', async () => {
            ...
            const mockCoreMemory = [{ label: 'memory1', value: longText }];
    ...
    describe('Error Handling', () => {
        it('should throw error for missing agent_id', async () => {
            await expect(handleGetAgentSummary(mockServer, {})).rejects.toThrow(
                'Missing required argument: agent_id',
            );

            await expect(handleGetAgentSummary(mockServer, { agent_id: null })).rejects.toThrow(
                'Missing required argument: agent_id',
            );

            await expect(handleGetAgentSummary(mockServer, { agent_id: '' })).rejects.toThrow(
                'Missing required argument: agent_id',
            );
        });
        ...
        it('should handle agent not found (404)', async () => {
            ...
            await expect(handleGetAgentSummary(mockServer, { agent_id: agentId })).rejects.toThrow(
